/**
 * BAHI.ar - Módulo de Búsqueda y Componentes
 * ──────────────────────────────────────────
 */

/**
 * Genera el HTML de una tarjeta de prestador.
 * @param {Object} p - Datos del prestador.
 * @param {string} modo - 'TURNO' o 'BUSQUEDA'.
 * @param {number} index - Índice para animaciones.
 * @returns {string} HTML de la tarjeta.
 */
window.crearCardHTML = function(p, modo = 'BUSQUEDA', index = 0) {
    const delay = index * 0.05;
    const isTurno = modo === 'TURNO';
    
    // Formatear teléfonos
    const telFijo = p.FIJO ? p.FIJO.replace('tel:', '') : '';
    const waUrl = p.MOVIL && p.MOVIL.includes('wa.me') ? p.MOVIL : '';
    
    return `
        <div class="card" ${isTurno ? 'data-estado="turno"' : ''} style="animation-delay: ${delay}s">
            <div class="card-top">
                <div class="card-info">
                    <h3 class="card-name">${p.PRESTADOR}</h3>
                    <p class="card-addr">${p.DOMICILIO} · ${p.LOCALIDAD}</p>
                </div>
                <div class="card-icon" aria-hidden="true">
                    ${isTurno ? '💊' : (p.TIPO === 'FARMACIA' ? '🏥' : '🔬')}
                </div>
            </div>
            
            <div class="card-actions">
                ${p.FIJO ? `
                    <a href="${p.FIJO}" class="btn btn-accent">
                        <svg class="btn-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/>
                        </svg>
                        Llamar
                    </a>
                ` : ''}
                
                ${waUrl ? `
                    <a href="${waUrl}" target="_blank" class="btn btn-whatsapp">
                        <svg class="btn-icon" viewBox="0 0 448 512" fill="currentColor">
                            <path d="M380.9 97.1C339 55.1 283.2 32 223.9 32c-122.4 0-222 99.6-222 222 0 39.1 10.2 77.3 29.6 111L0 480l117.7-30.9c32.4 17.7 68.9 27 106.1 27h.1c122.3 0 224.1-99.6 224.1-222 0-59.3-25.2-115-67.1-157zm-157 341.6c-33.2 0-65.7-8.9-94-25.7l-6.7-4-69.8 18.3L72 359.2l-4.4-7c-18.5-29.4-28.2-63.3-28.2-98.2 0-101.7 82.8-184.5 184.6-184.5 49.3 0 95.6 19.2 130.4 54.1 34.8 34.9 56.2 81.2 56.1 130.5 0 101.8-84.9 184.6-186.6 184.6zm101.2-138.2c-5.5-2.8-32.8-16.2-37.9-18-5.1-1.9-8.8-2.8-12.5 2.8-3.7 5.6-14.3 18-17.6 21.8-3.2 3.7-6.5 4.2-12 1.4-5.5-2.8-23.2-8.5-44.2-27.3-16.4-14.6-27.4-32.7-30.6-38.2-3.2-5.6-.3-8.6 2.4-11.3 2.5-2.4 5.5-6.5 8.3-9.7 2.8-3.3 3.7-5.6 5.5-9.3 1.8-3.7 .9-6.9-.5-9.7-1.4-2.8-12.5-30.1-17.1-41.2-4.5-10.8-9.1-9.3-12.5-9.5-3.2-.2-6.9-.2-10.6-.2-3.7 0-9.7 1.4-14.8 6.9-5.1 5.6-19.4 19-19.4 46.3 0 27.3 19.9 53.7 22.6 57.4 2.8 3.7 39.1 59.7 94.8 83.8 13.2 5.7 23.5 9.2 31.6 11.8 13.3 4.2 25.4 3.6 35 2.2 10.7-1.5 32.8-13.4 37.4-26.4 4.6-13 4.6-24.1 3.2-26.4-1.3-2.5-5-3.9-10.5-6.6z"/>
                        </svg>
                        WhatsApp
                    </a>
                ` : ''}
                
                <a href="${p.MAPS}" target="_blank" class="btn btn-outline">
                    <svg class="btn-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/>
                        <circle cx="12" cy="10" r="3"/>
                    </svg>
                    Mapa
                </a>
            </div>
            
            <button class="card-header" onclick="this.nextElementSibling.classList.toggle('open'); this.setAttribute('aria-expanded', this.nextElementSibling.classList.contains('open'))">
                <span>Más información</span>
                <span class="card-chevron">▼</span>
            </button>
            
            <div class="card-body-collapse">
                <div class="card-detail-row">
                    <strong>Horario:</strong> ${p.HORARIO || 'No especificado'}
                </div>
                ${p.STOCK ? `<div class="card-detail-row"><strong>Stock:</strong> ${p.STOCK}</div>` : ''}
                ${p.OOSS ? `<div class="card-detail-row"><strong>Obras Sociales:</strong> ${p.OOSS}</div>` : ''}
                <div class="card-detail-row">
                    <strong>Tipo:</strong> ${p.TIPO}
                </div>
            </div>
        </div>
    `;
};

/**
 * Lógica del Buscador
 */
(function() {
    let prestadores = [];
    const searchInput = document.getElementById('search-input');
    const contenedor = document.getElementById('contenedor-cards');
    const tituloHome = document.getElementById('titulo-home');
    const metaHome = document.getElementById('leyenda-horario');
    
    // Guardar estado original para volver cuando el buscador esté vacío
    let htmlOriginal = '';
    let tituloOriginal = '';
    let metaOriginal = '';

    async function cargarDatos() {
        try {
            // Intentar cargar desde diferentes rutas posibles
            const rutas = ['bd_bahiar.json', 'data/bd_bahiar.json', './bd_bahiar.json'];
            let res;
            for (const ruta of rutas) {
                res = await fetch(ruta);
                if (res.ok) break;
            }
            
            if (!res || !res.ok) throw new Error('No se pudo encontrar el archivo JSON');
            
            const data = await res.json();
            prestadores = data.prestadores;
            console.log('BAHI.ar: Datos cargados correctamente para búsqueda');
        } catch (e) {
            console.error('BAHI.ar: Error cargando prestadores:', e);
        }
    }

    function realizarBusqueda(query) {
        if (!query.trim()) {
            // Restaurar estado original
            if (htmlOriginal) {
                contenedor.innerHTML = htmlOriginal;
                tituloHome.textContent = tituloOriginal;
                metaHome.style.display = 'block';
            }
            return;
        }

        // Guardar estado si es la primera vez que escribimos
        if (!htmlOriginal) {
            htmlOriginal = contenedor.innerHTML;
            tituloOriginal = tituloHome.textContent;
            metaOriginal = metaHome.textContent;
        }

        const q = query.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
        
        const filtrados = prestadores.filter(p => {
            const nombre = p.PRESTADOR.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
            const domicilio = p.DOMICILIO.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
            const localidad = p.LOCALIDAD.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
            
            return nombre.includes(q) || domicilio.includes(q) || localidad.includes(q);
        });

        // Actualizar UI
        tituloHome.textContent = `Resultados (${filtrados.length})`;
        metaHome.style.display = 'none'; // Ocultar "Hasta las 8:30..."

        if (filtrados.length === 0) {
            contenedor.innerHTML = `
                <div class="error-state">
                    <div class="error-state-icon">🔍</div>
                    <p class="error-state-title">No hay resultados</p>
                    <p class="error-state-desc">No encontramos nada que coincida con "${query}"</p>
                </div>
            `;
        } else {
            contenedor.innerHTML = filtrados
                .map((p, i) => window.crearCardHTML(p, 'BUSQUEDA', i))
                .join('');
        }
    }

    // Eventos
    if (searchInput) {
        searchInput.addEventListener('input', (e) => realizarBusqueda(e.target.value));
        // También escuchar el evento 'keyup' por si acaso
        searchInput.addEventListener('keyup', (e) => realizarBusqueda(e.target.value));
        console.log('BAHI.ar: Buscador vinculado correctamente');
    } else {
        console.error('BAHI.ar: No se encontró el elemento #search-input');
    }

    // Cargar datos al inicio
    cargarDatos();
})();
