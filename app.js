...
const TIPO_RUTA_MAP = {
    ...
    'UNIDAD SANITARIA': 'guardias.html',
...
/**
 * Centraliza envío de eventos a Google Tag Manager
 */
function pushGTM(event, action) {
...
window.getSvgIcon = (campo) => {
    ...
    if (!svgName) return '';
    const url = `assets/cuadros/${svgName}.svg`;
...
function configurarBuscador() {
    ...
    inputs.forEach(input => {
        input.addEventListener('input', (e) => {
            ...
            debounceTimer = setTimeout(() => ejecutarBusqueda(valor), DEBOUNCE_DELAY);
        });
    });
}
...
async function buscarGlobal(filtro, contenedor) {
    try {
        if (!__bdCache) {
            ...
            __bdCache = await response.json();
        }
        if (!__ortoCache) {
            try {
                const responseOrto = await fetch('data/ortopedia.json');
                __ortoCache = responseOrto.ok ? await responseOrto.json() : { prestadores: [] };
            } catch (e) {
                __ortoCache = { prestadores: [] };
            }
        }
        const prestadoresOrto = (__ortoCache.prestadores || []).map(item => ({
            ...item,
            TIPO: item.TIPO || 'ORTOPEDIA'
        }));
        const prestadores = [...(__bdCache.prestadores || []), ...prestadoresOrto];
        ...
        document.querySelectorAll('.resultado-busqueda').forEach((el) => {
            el.addEventListener('click', () => {
                ...
                const url = TIPO_RUTA_MAP[tipo] || 'farmacias.html';
...
function configurarBotonCompartir() {
    ...
    btnCompartir.addEventListener('click', () => {
        ...
        mensaje += MENSAJE_WHATSAPP_FOOTER;
    ...
    document.addEventListener('click', async (e) => {
        ...
        if (btnInstalar) {
            ...
            try {
                ...
                pushGTM('pwa_event', outcome === 'accepted' ? 'instalacion_aceptada' : 'instalacion_rechazada');
